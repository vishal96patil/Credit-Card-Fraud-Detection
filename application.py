...
model = load(open('./Models/modell.pkl', 'rb'))
import numpy as np
...
def send():
    ...
    if prediction == 0:
        prediction_text = "Transaction Successful. No Fraud Detected. Redirecting "
        return render_template('verifytransaction.html', result=prediction_text), {"Refresh": "5; url=/checkout/"}

    elif prediction == 1:
        prediction_text = "Purchase cannot Be processed. Transaction Barred. Please Contact Admin. Ph: 9146492411"
        return render_template('verifytransaction.html', result=prediction_text), {"Refresh": "5; url=/fraudtrans/"}
...
@app.route("/verify/")
def verify():
     return render_template('verifytransaction.html')
...
@app.route("/fraudtrans/")
def fraud():
    order = db.execute("SELECT * from cart")
    # Update purchase history of current customer
    for item in order:
        db.execute("INSERT INTO fraud (uid, id, team, image, quantity) VALUES(:uid, :id, :team, :image, :quantity)", uid=session["uid"], id=item["id"], team=item["team"], image=item["image"], quantity=item["qty"] )
    # Clear shopping cart
    db.execute("DELETE from cart")
    shoppingCart = []
    shopLen = len(shoppingCart)
...
def fraudpanel():
    ...
    myShirts = db.execute("SELECT * FROM fraud")
    print(myShirts)
    myShirtsLen = len(myShirts)
    # Render table with shopping history of current user
    return render_template("admin.html", shoppingCart=shoppingCart, shopLen=shopLen, total=total, totItems=totItems, display=display, session=session, myShirts=myShirts, myShirtsLen=myShirtsLen)
